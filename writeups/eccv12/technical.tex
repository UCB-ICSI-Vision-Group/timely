...
Instead, we use Loopy Belief Propagation, which does not provide general convergence guarantees but has been shown to work well empirically on similar tasks \todo{cite}.

\todo{what to say about cost of inference? realistically it's 0.5 seconds while the detection action + classification is about 2 seconds.}
...
The final evaluation of a policy consists of the area under the performance vs. time curve.
...
R(b^j,a_i) = \frac{\Delta \text{ap}_i (t_T^j-\frac{1}{2}\Delta t_i)}{(1-\text{ap}_i)t_T^j}
\end{equation}
...
We also consider a similar additive reward function based on the mean entropy of the variables $P(C_k)$.
As with the above policy, the decrease in entropy is considered with regard to the time of the action.
...
\todo{Describe the greedy thing and the generalized policy iteration that we do.}
