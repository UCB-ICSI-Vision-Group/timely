...
\begin{center}
  ...
  \begin{tikzpicture}
    ...
    %% TODO: CAN GET 1CM AT TOP HERE  (when the number is -2)
    \setcounter{title-top}{\arabic{total-height}-2} % Places the top of the title 3 cm from the top of the poster.
    ...
    \path (\arabic{title-left},\arabic{title-top}) node(title) [style=tbox, text width=\arabic{title-width}cm] {
      \begin{center}
        ...
        \parbox{10cm} {
        \includegraphics[height=8cm]{../../figures/berkeley_seal.pdf}
        }
    ...
    \textbf{\textsc{Problem}}
    \begin{itemize}
      \item Want to detect objects of many classes in an image.
      \item Detectors and classifiers already exist, but there is \textbf{not enough time to run all detectors}.
    \end{itemize}
    ...
    \path (motivation.south west) ++(0cm,-2cm) node(implementation) [style=tbox,text width=28cm] {
      \parbox{16cm}{
      \textbf{\textsc{AP vs Time and the Reward}}
      \begin{itemize}
        \item The final performance evaluation is the area under the AP vs. Time curve between $T_s$ and $T_d$.
        \item Because this is additive per action, we define \begin{align*} R(s^j,a) = \Delta \text{ap} (t_T^j-\frac{1}{2}\Delta t) \end{align*}
      \end{itemize}
      }
      \parbox{12cm}{\hfill\includegraphics[width=11cm]{../../figures/apvst_expl.pdf}}
      \\

      \textbf{\textsc{Learning $\pi(s)$}} with fitted Q-iteration iteration.
      \begin{itemize}
      ...
      \begin{itemize}
      \item $\gamma=0$ induces a completely greedy policy, and at $\gamma=1$ is full lookahead.
      \item $\gamma = 0.4$ performs best in cross-validation on the final evaluation.
      \end{itemize}
      \vspace{1cm}

      \textbf{\textsc{The belief state}} posterior $P(C_k|\mathbf{o})$ is updated with observations $\mathbf{o}$ of detector for class $k$.
      \begin{itemize}
        \item \emph{Direct} method simply replaces the posterior of the corresponding class(es).
        \item \emph{MRF} (pictured) method runs loopy BP in an MRF model, updating all posteriors.
        \item MRF weights are learned with $L_1$-regularization.
      \end{itemize}
    ...
    \path (motivation.north east) ++(2cm,0cm) node(figure1) [style=tbox,text width=38cm] {
      ...
      \begin{itemize}
        ...
        \item We define our policy as taking an untaken action with maximum value. \begin{align*} \pi(s) = \argmax{a_i \in \mathcal{A} \setminus \mathcal{O}} Q(s,a_i) = \theta_\pi^\top \phi(s,a) \end{align*}
        \item Must learn an accurate approximation of the value function \begin{align*}Q^\pi(s^j,a) = \mathbb{E} [R \mid s, a, \pi)]\end{align*} where $R = \sum_{i=j}^J \gamma^{i-j} R(s^i,a^i)$ and $J$ is the index of the last action before deadline time $T_d$.
      \end{itemize}
...
%%% FEATURES AND WEIGHTS
    \path (figure1.south west) ++(0cm,-2cm) node(features) [style=tbox,text width=38cm] {
      The features $\phi(s, a)$ reflect
      \begin{itemize}
        \item the probability that class $C$ is present in the image and its degree of certainty, for all classes.
        \item time to $T_s$ and to $T_d$, and the expected time of the action.
      \end{itemize}
      \vspace{5mm}
...
%%% RESULTS
    \path (figure1.north east) ++(2cm,0cm) node(results) [style=tbox,text width=28cm] {
      ...
      \begin{center}\textbf{Policy Trajectories}\end{center}

      \parbox{20cm}{
      \includegraphics[width=19cm]{../../figures/trajectories.pdf}}%\hfill
      \parbox{10cm}{
