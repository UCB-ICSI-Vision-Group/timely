  ...
  \begin{tikzpicture}
    ...
    \path (\arabic{title-left},\arabic{title-top}) node(title) [style=tbox, text width=\arabic{title-width}cm] {
      \begin{center}
        ...
        \parbox{10cm} {
        \includegraphics[height=9cm]{../../figures/berkeley_seal.pdf}
        }
    ...
    \begin{itemize}
      ...
      \item We use reinforcement learning to learn a \emph{dynamic} policy for selecting detectors and classifiers, and learn to take actions that do not provide immediate reward.
    ...
    \path (introduction.south west) ++(0cm,-2cm) node(coding) [style=tbox,text width=30cm] {
      \begin{center}
        \includegraphics[width=24cm]{../../figures/pomdp_annotated.pdf}
      \end{center}

      \begin{itemize}
        \item An action $a \in \mathcal{A}$ can run detection for an object class $k$ in the whole image, or classify the scene.
        \item Potential actions are considered according to their predicted \emph{value} $Q(s,a)$, which also depends on the \emph{belief state} $s$ of the system.
        \item The selected action returns observations in the form of a list of detections, or simply an evaluated feature.
    ...
    \path (introduction.north east) ++(2cm,0cm) node(model) [style=tbox,text width=34 cm] {
      \begin{center}
        \includegraphics[width=30cm]{../../figures/figure1_extended_hor.pdf}
      \end{center}
      ...
      The value function
      \begin{align*}Q^\pi(s^j,a) = \mathbb{E} [R \mid s, a, \pi)]\end{align*}
      where $R = \sum_{i=j}^J \gamma^{i-j} R(s^i,a^i)$ and $J$ is the index of the last action before deadline time $T_d$.

      \parbox{20cm}{
      \textbf{Reward Function:} The final performance evaluation of area under the AP vs. Time curve between $T_s$ and $T_d$ is additive per action.
      ...
      To compute , we run Monte Carlo policy iteration.
    ...
    }; \addcenteredtitle{model}{Learning to Detect on a Budget}
